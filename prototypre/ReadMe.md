...
- 실험 도중 센서로 각속도를 추출 후 PI제어기에 넣고 돌린 모델이 기존 제어기보다 훨씬 안정적으로 동작하는 것을 확인했다. 이 경향은 특히 안티 와인드업을 집어넣었을 때 보다   크게 두드러졌다. 따라서 실제 로봇에 제어기를 대입할 때에는 PID제어기보다는 PI 제어기가 더 효과적일 것으로 보인다.  
------------------------------------------------------------------
# 4.20(KJH)
- 여러 시뮬레이션을 진행하면서 타카키 교수님의 논문에 있는 제어기 식은 matlab simulink의 PID tuner를 사용한 튜닝 방식에 별로 적합하지 않음을 확인했다.(제어가 잘 안됨)
- 그동안 하드웨어 제작과 동시에 여러 시뮬레이션을 진행해본 결과 각도값과 각속도 값을 통한 PI제어가 가장 효과적으로 동작하는 것을 확인했다. 
- 모터의 기어박스(1:50)를 고려하지않고 모터의 토크상수를 구했던 점을 수정함 
- 기어박스에 따른 토크상승과 전류값 제한에 따른 안티와인드업을 구성해 제어기를 설계해본 결과 계단을 올라가는데 토크가 충분한 것으로 파악된다.
- 변수는 시뮬레이션상의 로봇의 무게와 실제 로봇의 무게의 차가 될 것으로 보인다. 이 부분을 고치기위해 실제 로봇을 제작후 다시 시뮬레이션 모델을 로봇의 무게에 맞게 바꿔야 할 것이다.
- 그동안 진행했던 시뮬레이션 파일 수정본을 업로드했다(4.18,4.19)

# 5.16(KJH)
- 하드웨어의 전반적인 형태를 완성한 뒤 각 부품의 무게를 측정함
- 각 부품의 무게를 simulink의 각 모델 부품 -> inertia -> center of mass 에 값으로 기입함
- 이를 통해 시뮬레이션의 정확도과 실제와 비슷하게 바뀔 것으로 기대됨
- 현재 내가 고려하고 있는 제어기는 두가지로 센서의 각속도와 칼만 필터를 거친 각도를 가져와 각각에 게인값을 붙여 더해 제어입력을 구성하는 PI제어기와 칼만 필터를 거쳐서 만든 각도에러만을 이용한 PID제어기 두가지이다
- 이중 일단 PID제어기를 통해 내부 프로그램을 실험해볼 예정이디.  
